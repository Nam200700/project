...
public class DarkLightSwitchIcon implements AnimatedIcon {
    ...
    private Icon darkIcon = new FlatSVGIcon("drawer/svg/dark.svg", 0.75f);
    private Icon lightIcon = new FlatSVGIcon("drawer/svg/sun.svg", 0.75f);
    ...
    public void paintIconAnimated(Component c, Graphics g, int x, int y, float animatedValue) {
        ...
        float arc = Math.min(getBorderArc(c) * 4f, size);
        float animatedX = x + (width - size) * animatedValue;

        g2.setColor(color);
        g2.fill(new RoundRectangle2D.Double(x, y, width, size, arc, arc)); // Chỉnh chiều rộng + bo góc
    ...
    public int getIconWidth() {
        return darkIcon.getIconWidth() + lightIcon.getIconWidth() + UIScale.scale(centerSpace) + UIScale.scale(iconGap) * 12;
    }
